(function( $ ) {
	...
	let settings = {
	  // "async": true,
	  // "crossDomain": true,
	  // "url": "http://21.21.21.5:4000/listarDespesas",
	...
	$.ajax(settings).done(function (response) {
	  ...
	  // console.log(response);
	  	...
	  	console.log(value.data_ent, value.despesa_ent, value.valor_ent );

	  		$( '#textareaDisp' ).append (
																					'<tr>' + '<td width="22%">' + value.data_ent + '</td>' +
																						'<td width="48%">' + value.despesa_ent + '</td>' +
																						'<td width="30%">' + "R$ " + 
																						parseFloat(value.valor_ent).toFixed( 2 ).replace('.',',') +
																						'</td>' +	'</tr>'
																					);
	  })
