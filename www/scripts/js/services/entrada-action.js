angular
  .module("entradaServices", ["ngResource", "routesApp"])
  .factory(
    "entradaAction",
    function ($q, routesAction, formatValor, formatDate) {
      let service = {};
      ...
      service.index = (getDados) => {
        return $q((res, rej) => {
          try {
            ...
            rej({
              message: "Error: FACTORY ENTRADA SERVICE, falha ao buscar dados",
            });
          }
        });
      };
      service.indexMesAnterior = (getDados) => {
        return $q((res, rej) => {
          try {
            res(
              routesAction.entradaIndex(FormatDataBuscaMesAnterior(getDados))
            );
          } catch {
            rej({
              message:
                "Error: FACTORY ENTRADA SERVICE, falha ao buscar dados do mês anterior",
            });
          }
        });
      };
        ...
        return $q((res, rej) => {
          try {
            ...
            for (let row of getDados) {
              ...
              dadosFormat = [formtDate, formtDate, row.entrada, row.valor];
              routesAction.entradaCreate(dadosFormat);
            }
            res({
              message:
                "Success: Entrada Caixa adicionadas com base no mês anterior",
            });
          } catch {
            rej({
              message:
                "Error: FACTORY ENTRADA SERVICE, falha ao copiar dados do mês anterior",
            });
          }
        });
      };
      service.create = (getDados) => {
        return $q((res, rej) => {
          try {
            ...
            rej({
              message:
                "Error: FACTORY ENTRADA SERVICE, falha ao cadastra entrada",
            });
          }
        });
      };
      service.update = (getDados) => {
        return $q((res, rej) => {
          try {
            ...
            rej({
              message:
                "Error: FACTORY ENTRADA SERVICE, falha ao atualizar entrada",
            });
          }
        });
      };
      service.delete = (getDados) => {
        return $q((res, rej) => {
          try {
            ...
            rej({
              message: "Error: FACTORY ENTRADA SERVICE, falha excluir entrada",
            });
          }
        });
      };
