angular.module('relatorioServices', ['ngResource'])
.factory('relatorioAction', function($q, getDadosAction, formatDadosAction, alertAction){
      ...
      try{
        getDadosAction.index().then((response) => {
          formatDadosAction.index(response).then((responseFormat) => {
            ...
            responseFormat.listaDespesas = services.FormatDadosViewRelatorio(responseFormat.resultValorTotal.despesaValorTotal, responseFormat.listaDespesas)

            responseFormat.listaInvestimentos = services.FormatDadosViewRelatorio(responseFormat.resultValorTotal.entradaValorTotal, responseFormat.listaInvestimentos)
            
            responseFormat.graphicInvestimento = {
              nome: 'Investimento',
              valor: responseFormat.resultValorTotal.investimetosValorTotal,
        ...
        rej({message: 'Erro: FACTORY RELATÓRIO SERVICES, falha ao BUSCAR os dados relatório'})
      }
    })
  }
