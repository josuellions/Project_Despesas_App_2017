angular
  .module("despesaServices", ["ngResource", "routesApp"])
  .factory(
    "despesaAction",
    function ($q, $rootScope, routesAction, formatValor) {
      let service = {};
      ...
      service.index = (getDados) => {
        return $q((res, rej) => {
          ...
          rej({
            message: "Error: FACTORY DESPESA SERVICES, falha ao buscar dados",
          });
        });
      };
      service.indexSemInvestimentos = (getDados) => {
        return $q((res, rej) => {
          res(routesAction.despesaIndexSemInvestimentos(getDados));
          rej({
            message: "Error: FACTORY DESPESA SERVICES, falha ao buscar dados",
          });
        });
      };
      service.indexInvestimentos = (getDados) => {
        return $q((res, rej) => {
          res(routesAction.despesaIndexInvestimentos(getDados));
          rej({
            message: "Error: FACTORY DESPESA SERVICES, falha ao buscar dados",
          });
        });
      ...
      service.indexMesAnterior = (getDados) => {
        ...
        const dadosFormat = [
          ...
          rej({
            message:
              "Error: FACTORY DESPESA SERVICES, falha ao buscar dados mês anterior",
          });
        });
      };
      service.copy = (getDados) => {
        return $q((res, rej) => {
          ...
          $.each(getDados, (id, row) => {
            ...
            const formtDate = GetDateFormat.anoFullMesDiaFormatBDParamsFull(
              date[2],
              parseInt(date[1]),
              date[0]
            );
          ...
          res({
            message: "Success: Despesas adicionadas com base no mês anterior",
          });
          rej({
            message:
              "Error: FACTORY DESPESA SERVICES, falha ao copiar dados mês anterior",
          });
        });
      };
      service.create = (getDados) => {
        return $q((res, rej) => {
          ...
          rej({
            message:
              "Error: FACTORY DESPESA SERVICES, falha ao cadastra despesa",
          });
        });
      };
      service.update = (getDados) => {
        return $q((res, rej) => {
          try {
            ...
            rej({
              message:
                "Error: FACTORY DESPESA SERVICES, falha ao atualizar despesa",
            });
          }
        });
      };
      service.updateStatus = (getDados) => {
        return $q((res, rej) => {
          ...
          res(
            routesAction.despesaStatus([
              ConvertStatus(getDados.status),
              getDados.id,
            ])
          );
          rej({
      ...
      service.delete = (getDados) => {
        return $q((res, rej) => {
          ...
          rej({
            message:
              "Error: FACTORY DESPESA SERVICES, falha ao excluir despesa",
          });
        });
      };
      return service;
