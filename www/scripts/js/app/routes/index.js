  ...
  app.backupLocal = () => {
    return $q((res, rej) => {
      try{
        ...
        rej({message: 'Error: FACTORY ROUTE SERVICES, falha ao buscar dados Local'})
      }
    })
  }
  app.conectionAPI = (getConextion) => {
    return $q((res, rej) => {
      try{
        ...
        rej({message: 'Error: FACTORY ROUTE SERVICES, falha conexão com API'})
      }
    })
  }
  app.backupAPI = (getConextion) => {
    return $q((res, rej) => {
      try{
        res(api.downloadAPI(getConextion))
      }
      catch{
        rej({message: 'Error: FACTORY ROUTE SERVICES, falha busca dados na API'})
      }
    })
  }
