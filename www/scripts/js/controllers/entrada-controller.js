  ...
  .controller(
    ...
    function (
      ...
      const CopyMesAnterior = (getDataInicio, response) => {
        ...
        const dtMesAnteriorFormat = GetDateFormat.mesExtAnoParams(
        ...
        alertAction
          .question(
            `Adicionar entrada caixa, com base no mês anterior, "${dtMesAnteriorFormat}"?`,
            action.copyEntrada,
            { dados: response, date: dtMesAnteriorFormat }
          )
          .catch((err) => {
      ...
      const BuscarDadosMesAnterior = (getDataInicio) => {
        entradaAction
          .indexMesAnterior(getDataInicio)
          .then((res) => {
            //Exclui saldo mês anterior calculo automático
            if (res[0].id === 0) {
              res.splice(res.indexOf(res[0]), 1);
            }
      ...
      const Actions = () => {
        ...
        const indexEntrada = () => {
          formatDate
            ...
            .then((response) => {
              entradaAction
                .index([response.inicio, response.fim])
                .then((res) => {
                  //Exclui saldo mês anterior calculo automático
                  if (res[0].id === 0 && res.length === 1) {
                    res.splice(res.indexOf(res[0]), 1);
                  }
        ...
        const copyEntrada = (getDados) => {
          entradaAction
            ...
            .then((res) => {
              ListaComTimeOut();
              alertAction
                .info(`${res.message} "${getDados.date}"`)
                .catch((err) => {
                  alertAction.error(err.message).catch((err) => {
                    alert(err.message);
                  });
      ...
      $scope.submitDelete = (entrada) => {
        alertAction
          .question(
            `Deseja excluir entrada caixa "${entrada.nome}"?`,
