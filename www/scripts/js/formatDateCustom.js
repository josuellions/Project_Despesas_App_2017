...
const basemesPag = () => {
  //1x usado InsertMesBaseAnterior
...
const GetDateFormat = {
  ...
  anoFullMesExtDiaParams: (mesExt, ano) => {
    //yyyy-MM-dd
...
const formatDataInsert = () => {
  ...
  dtMes == 0 ? ((dtMes = 12), (dtAno = dtAno - 1)) : false;
...
const FormatDataBuscaMesAnterior = (getDate) => {
  ...
  const dateFormat = [
    GetDateFormat.anoFullMesDiaFormatBDParamsFull(
      1,
      date.getMonth(),
      date.getFullYear()
    ),
...
const FormatDateParaBancoDados = (getDate) => {
  const date = new Date(getDate);
  return GetDateFormat.anoFullMesDiaFormatBDParamsFull(
    date.getDate(),
    date.getMonth(),
    date.getFullYear()
  );
